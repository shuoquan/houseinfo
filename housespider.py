...
def conn_db():  #数据库连接函数
    ...
    cur=conn.cursor()
    return conn,cur
class HouseSpider(Thread):
    def __init__(self,url,conndb,cur,q):
    ...
    def getHouseInfo(self,url, conndb, cur):
        ...
        for houseurl in houseurls:
            ...
            # print(info)
            # sql="insert into test(name TEXT,age TEXT) VALUES ('%s','%s')%(tom,12)"

            sql = "insert into sechouse(小区名称,所在区域,房屋户型,建筑面积,单价,总价,房屋朝向,所在楼层,装修情况)VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
...
#
conn,cur=conn_db()
# sql = "create table if not exists sechouse(小区名称 TEXT,所在区域 TEXT,房屋户型 TEXT,建筑面积 TEXT,单价 TEXT,总价 TEXT,房屋朝向 TEXT,所在楼层 TEXT,装修情况 TEXT)"
# cur.execute(sql)
domain="https://sh.lianjia.com/"
q=Queue()
Thread_list=[]
urllist=getAllurl(1,4,domain)
for url in urllist:
    p=HouseSpider(url,conn,cur,q)
    p.start()
